...
#Several functions to go from HW-like to float human readable represesntation
def getEta(etaHW):
        ...
        for gen in gens:
            ...
            for mu in goodmuons:
                ...
                if v_mu.DeltaR( v_gen ) < 0.5 and mu.hwQual() >= 12: 
                    ...
                    rematchedPt = v_mu.Pt() 
                    passes_0_12 = True
                    if passes_20_12: break # If we pass this we pass all

            # Then fill all the matched histos
...
import pickle 
with open('tdr_eff_pt3_PU200.pickle', 'wb') as handle:
