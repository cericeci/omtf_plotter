...
#Datasets to read
datasets = { 
    'Nu_PU140'       : { 'path' : '/eos/cms/store/group/phys_muon/sesanche/overlap_sep20/Nu_E10-pythia8-gun/Nu_PU140/190920_153934/', 'total':0              },
    ...
    'Nu_PU250'       : { 'path' : '/eos/cms/store/group/phys_muon/sesanche/overlap_sep20/Nu_E10-pythia8-gun/Nu_PU250/190920_153635/', 'total':0              },    
    'Nu_PU300'       : { 'path' : '/eos/cms/store/group/phys_muon/cericeci/overlap_sep20/Nu_E10-pythia8-gun/Nu_PU300/190926_134749/', 'total':0              },  
}

#These are the edges of the pT bins to check 
    ...
    for ev in events:
        ...
        for bxNumber in range(muons.getFirstBX(), muons.getLastBX()+1):
            ...
            for i in range(size):
                ...
                for b in bins:
                    if getPt(muon.hwPt()) >= b: #Notice that we fill already the cumulative histogram
                        if muon.hwQual() >= 12:
...
import pickle 
with open('tdr_rates_pu.pickle', 'wb') as handle:
